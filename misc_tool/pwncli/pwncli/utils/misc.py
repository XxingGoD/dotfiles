...
def reveal_heapbase(mangled, pos=None, offset=0, null_fd=False):
    if null_fd:
        ptr = pos if pos is not None else (mangled << 12)
    else:
        ptr = (mangled ^ (pos >> 12)) if pos is not None else reveal_ptr(mangled)
